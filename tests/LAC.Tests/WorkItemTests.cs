...
public sealed class WorkItemTests : IClassFixture<WorkItemTestFactory>
{
    ...
    public async Task Phase2FC_BranchPulse_AttentionFilters_UseExactDerivedSemantics()
    {
        ...
        Assert.Contains(ids["overdue"], await Filter("overdue")); Assert.Contains(ids["today"], await Filter("due-today")); Assert.Contains(ids["review"], await Filter("needs-review")); Assert.DoesNotContain(ids["review"], await Filter("waiting-on-help")); Assert.Contains(ids["waiting"], await Filter("waiting-on-help")); Assert.Contains(ids["notstarted"], await Filter("not-started")); Assert.Contains(ids["nohandler"], await Filter("no-named-handler")); Assert.Contains(ids["stale"], await Filter("stale")); Assert.DoesNotContain(ids["terminal"], await Filter("open"));
    }
