...
public sealed class WorkItemTests : IClassFixture<WorkItemTestFactory>
{
    ...
    public async Task Phase2FC_BranchPulse_Aggregates_CurrentDesk_And_Excludes_Terminal()
    {
        ...
        var prefix = "P2FC-PULSE-" + Guid.NewGuid().ToString("N"); using (var scope = _factory.Services.CreateScope()) { var db = scope.ServiceProvider.GetRequiredService<LacDbContext>(); foreach (var status in new[] { WorkItemStatus.Assigned, WorkItemStatus.InProgress, WorkItemStatus.Completed }) { var item = new WorkItem { Id = Guid.NewGuid(), WorkstreamId = ws.Id, Title = prefix + status, Status = status, Priority = WorkItemPriority.Urgent, RequestedByUserId = SeedData.BootstrapAdminId, RequestedByDisplayNameSnapshot = "Admin", Revision = 1, LastActivityAt = DateTimeOffset.UtcNow.AddDays(-8), RecordStatus = RecordStatus.Active }; db.Add(item); db.Add(new WorkItemAssignment { Id = Guid.NewGuid(), WorkItemId = item.Id, OfficeDeskId = desk.Id, IsActive = true, AssignedAt = DateTimeOffset.UtcNow, RecordStatus = RecordStatus.Active }); } await db.SaveChangesAsync(); }
        var response = await client.GetAsync($"/api/work-items/branch-pulse?attention=open&staleDays=7&q={prefix}"); Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    ...
    public async Task Phase2FC_Reassign_Closes_Cycle_And_Emits_Immutable_Snapshots()
    {
        ...
        var assignedAt = DateTimeOffset.UtcNow.AddHours(-3); var firstSeen = assignedAt.AddHours(1); var firstAction = assignedAt.AddHours(2); var lastActivity = DateTimeOffset.UtcNow.AddMinutes(-1);
        using (var scope = _factory.Services.CreateScope()) { var db = scope.ServiceProvider.GetRequiredService<LacDbContext>(); var item = new WorkItem { Id = Guid.NewGuid(), WorkstreamId = ws.Id, Title = "Route", Status = WorkItemStatus.InProgress, RequestedByUserId = SeedData.BootstrapAdminId, RequestedByDisplayNameSnapshot = "Admin", Revision = 4, LastActivityAt = lastActivity, RecordStatus = RecordStatus.Active }; var old = new WorkItemAssignment { Id = Guid.NewGuid(), WorkItemId = item.Id, OfficeDeskId = oldDesk.Id, IsActive = true, AssignedAt = assignedAt, FirstSeenAt = firstSeen, FirstActionAt = firstAction, RecordStatus = RecordStatus.Active }; db.Add(item); db.Add(old); await db.SaveChangesAsync(); id = item.Id; oldId = old.Id; }
        ...
        using var scope2 = _factory.Services.CreateScope(); var db2 = scope2.ServiceProvider.GetRequiredService<LacDbContext>(); var cycles = await db2.WorkItemAssignments.Where(a => a.WorkItemId == id).ToListAsync(); Assert.Equal(2, cycles.Count); var oldCycle = cycles.Single(a => a.Id == oldId); Assert.False(oldCycle.IsActive); Assert.NotNull(oldCycle.ClosedAt); Assert.Equal(RecordStatus.Active, oldCycle.RecordStatus); Assert.Equal(assignedAt, oldCycle.AssignedAt); Assert.Equal(firstSeen, oldCycle.FirstSeenAt); Assert.Equal(firstAction, oldCycle.FirstActionAt); var current = cycles.Single(a => a.IsActive); Assert.NotEqual(oldId, current.Id); Assert.Null(current.FirstSeenAt); Assert.Null(current.FirstActionAt); Assert.Null(current.ClosedAt); Assert.Equal(SeedData.BootstrapAdminId, current.AssignedByUserId); Assert.Equal(newDesk.Id, current.OfficeDeskId); var itemAfter = await db2.WorkItems.FindAsync(id); Assert.Equal(WorkItemStatus.InProgress, itemAfter!.Status); Assert.Equal(ws.Id, itemAfter.WorkstreamId); Assert.Equal(5, itemAfter.Revision); Assert.True(itemAfter.LastActivityAt > lastActivity); Assert.Empty(await db2.WorkItemContributors.Where(c => c.WorkItemId == id).ToListAsync()); var audit = await db2.WorkItemEvents.SingleAsync(e => e.WorkItemId == id && e.Action == WorkItemEventAction.Reassigned); Assert.Equal(oldId, audit.SourceAssignmentId); Assert.Equal(current.Id, audit.TargetAssignmentId); Assert.Equal(oldDesk.Id, audit.SourceDeskId); Assert.Equal(newDesk.Id, audit.TargetDeskId); Assert.Equal("Court Desk", audit.SourceDeskNameSnapshot); Assert.Equal("Land Records", audit.TargetDeskNameSnapshot); Assert.Equal("Verification required", audit.RemarksSnapshot);
    }
    ...
    [Fact]
    public async Task Phase2FC_Reassign_InactiveTargetDesk_IsRejected()
    { var(client,id,_,target)=await SeedPhase2FCReassignAsync();using(var s=_factory.Services.CreateScope()){var db=s.ServiceProvider.GetRequiredService<LacDbContext>();(await db.OfficeDesks.FindAsync(target))!.IsActive=false;await db.SaveChangesAsync();}Assert.Equal(HttpStatusCode.BadRequest,(await client.PostAsJsonAsync($"/api/work-items/{id}/reassign",new ReassignWorkItemApiRequest(target,null,"No",1))).StatusCode); }

    [Fact]
    public async Task Phase2FC_Reassign_TargetUserMustBeLiveMemberOfTargetDesk()
    { var(client,id,_,target)=await SeedPhase2FCReassignAsync();var ws=await CreateWorkstreamAsync("RTU"+Guid.NewGuid().ToString("N")[..6],"User");var(_,user)=await CreateScopedUserClientAsync("rtuser"+Guid.NewGuid().ToString("N")[..6],"rt-role-"+Guid.NewGuid().ToString("N")[..6],ScopeMode.Workstream,workstreamId:ws.Id);Assert.Equal(HttpStatusCode.BadRequest,(await client.PostAsJsonAsync($"/api/work-items/{id}/reassign",new ReassignWorkItemApiRequest(target,user,"No",1))).StatusCode); }

    [Fact]
    public async Task Phase2FC_Reassign_InactiveTargetUser_IsRejected()
    { var(client,id,_,target)=await SeedPhase2FCReassignAsync();var ws=await CreateWorkstreamAsync("RIU"+Guid.NewGuid().ToString("N")[..6],"User");var desk=await CreateDeskAsync("RIUD"+Guid.NewGuid().ToString("N")[..6],"Member",ws.Id,assignAdmin:false);var(_,user)=await CreateScopedUserClientAsync("riuser"+Guid.NewGuid().ToString("N")[..6],"ri-role-"+Guid.NewGuid().ToString("N")[..6],ScopeMode.Workstream,deskId:desk.Id,workstreamId:ws.Id);using(var s=_factory.Services.CreateScope()){var db=s.ServiceProvider.GetRequiredService<LacDbContext>();var m=await db.UserDeskMemberships.SingleAsync(m=>m.UserId==user&&m.OfficeDeskId==desk.Id);m.OfficeDeskId=target;(await db.AppUsers.FindAsync(user))!.IsActive=false;await db.SaveChangesAsync();}Assert.Equal(HttpStatusCode.BadRequest,(await client.PostAsJsonAsync($"/api/work-items/{id}/reassign",new ReassignWorkItemApiRequest(target,user,"No",1))).StatusCode); }

    [Fact]
    public async Task Phase2FC_Reassign_ExactSameDeskAndHandler_IsRejectedAsNoOp()
    { var(client,id,source,_)=await SeedPhase2FCReassignAsync();var before=1;var res=await client.PostAsJsonAsync($"/api/work-items/{id}/reassign",new ReassignWorkItemApiRequest(source,null,"Same",before));Assert.Equal(HttpStatusCode.BadRequest,res.StatusCode);using var s=_factory.Services.CreateScope();var db=s.ServiceProvider.GetRequiredService<LacDbContext>();Assert.Single(await db.WorkItemAssignments.Where(a=>a.WorkItemId==id).ToListAsync());Assert.Empty(await db.WorkItemEvents.Where(e=>e.WorkItemId==id&&e.Action==WorkItemEventAction.Reassigned).ToListAsync());Assert.Equal(before,(await db.WorkItems.FindAsync(id))!.Revision); }

    [Fact]
    public async Task Phase2FC_Reassign_WorkstreamScope_Requires_LiveMembership()
    {
        var admin = await CreateAdminClientAsync(); var ws = await CreateWorkstreamAsync("RWS" + Guid.NewGuid().ToString("N")[..6], "Routing"); var source = await CreateDeskAsync("RWSS" + Guid.NewGuid().ToString("N")[..6], "Source", ws.Id, assignAdmin: false); var target = await CreateDeskAsync("RWST" + Guid.NewGuid().ToString("N")[..6], "Target", assignAdmin: false); var (caller, user) = await CreateCustomUserClientAsync("rwsu" + Guid.NewGuid().ToString("N")[..6], "rws-role-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Workstream, [PermissionCodes.WorkItemAssign], workstreamId: ws.Id); var id = await SeedPhase2FCPulseItemAsync(ws, source, "P2FC-RWS", SeedData.BootstrapAdminId);
        Assert.Equal(HttpStatusCode.OK, (await caller.PostAsJsonAsync($"/api/work-items/{id}/reassign", new ReassignWorkItemApiRequest(target.Id, null, "live", 1))).StatusCode);
        var id2 = await SeedPhase2FCPulseItemAsync(ws, source, "P2FC-RWS-REMOVED", SeedData.BootstrapAdminId); using (var s = _factory.Services.CreateScope()) { var db = s.ServiceProvider.GetRequiredService<LacDbContext>(); var m = await db.UserWorkstreamMemberships.SingleAsync(x => x.UserId == user && x.WorkstreamId == ws.Id); m.IsActive = false; await db.SaveChangesAsync(); }
        Assert.Equal(HttpStatusCode.Forbidden, (await caller.PostAsJsonAsync($"/api/work-items/{id2}/reassign", new ReassignWorkItemApiRequest(target.Id, null, "removed", 1))).StatusCode);
    }

    [Fact]
    public async Task Phase2FC_Reassign_AssignedScope_Uses_CurrentResponsibleDesk()
    {
        var ws = await CreateWorkstreamAsync("RAS" + Guid.NewGuid().ToString("N")[..6], "Assigned"); var desk = await CreateDeskAsync("RASD" + Guid.NewGuid().ToString("N")[..6], "Institution", ws.Id, assignAdmin: false); var target = await CreateDeskAsync("RAST" + Guid.NewGuid().ToString("N")[..6], "Target", assignAdmin: false); var (_, named) = await CreateCustomUserClientAsync("rasa" + Guid.NewGuid().ToString("N")[..6], "ras-a-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Workstream, [PermissionCodes.WorkItemView], deskId: desk.Id, workstreamId: ws.Id); var (caller, member) = await CreateCustomUserClientAsync("rasb" + Guid.NewGuid().ToString("N")[..6], "ras-b-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Assigned, [PermissionCodes.WorkItemAssign], deskId: desk.Id, workstreamId: ws.Id); var id = await SeedPhase2FCPulseItemAsync(ws, desk, "P2FC-RAS", SeedData.BootstrapAdminId);
        using (var s = _factory.Services.CreateScope()) { var db = s.ServiceProvider.GetRequiredService<LacDbContext>(); (await db.WorkItemAssignments.SingleAsync(x => x.WorkItemId == id && x.IsActive)).AssignedUserId = named; await db.SaveChangesAsync(); }
        Assert.NotEqual(named, member); Assert.Equal(HttpStatusCode.OK, (await caller.PostAsJsonAsync($"/api/work-items/{id}/reassign", new ReassignWorkItemApiRequest(target.Id, null, "institution", 1))).StatusCode);
    }

    [Fact]
    public async Task Phase2FC_Reassign_ContributorRelation_DoesNotGrantAssign()
    {
        var (admin, id, _, target) = await SeedPhase2FCReassignAsync(); var ws = await CreateWorkstreamAsync("RCO" + Guid.NewGuid().ToString("N")[..6], "Contributor"); var (caller, user) = await CreateCustomUserClientAsync("rcou" + Guid.NewGuid().ToString("N")[..6], "rco-role-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Assigned, [PermissionCodes.WorkItemView], workstreamId: ws.Id); using (var s = _factory.Services.CreateScope()) { var db = s.ServiceProvider.GetRequiredService<LacDbContext>(); db.WorkItemContributors.Add(new WorkItemContributor { Id = Guid.NewGuid(), WorkItemId = id, UserId = user, AddedByUserId = SeedData.BootstrapAdminId, AddedAt = DateTimeOffset.UtcNow, Status = WorkItemContributorStatus.Active, IsActive = true, RecordStatus = RecordStatus.Active }); await db.SaveChangesAsync(); }
        Assert.Equal(HttpStatusCode.Forbidden, (await caller.PostAsJsonAsync($"/api/work-items/{id}/reassign", new ReassignWorkItemApiRequest(target, null, "no authority", 1))).StatusCode);
    }

    [Fact]
    public async Task Phase2FC_Reassign_OwnScope_FailsClosed()
    { var (_, id, _, target) = await SeedPhase2FCReassignAsync(); var ws = await CreateWorkstreamAsync("ROW" + Guid.NewGuid().ToString("N")[..6], "Own"); var (caller, _) = await CreateCustomUserClientAsync("rowu" + Guid.NewGuid().ToString("N")[..6], "row-role-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Own, [PermissionCodes.WorkItemAssign], workstreamId: ws.Id); Assert.Equal(HttpStatusCode.Forbidden, (await caller.PostAsJsonAsync($"/api/work-items/{id}/reassign", new ReassignWorkItemApiRequest(target, null, "own", 1))).StatusCode); }

    [Fact]
    public async Task Phase2FC_Reassign_DoesNotUse_DeskWorkstream_AsRoutingRestriction()
    {
        var client = await CreateAdminClientAsync(); var a = await CreateWorkstreamAsync("RCA" + Guid.NewGuid().ToString("N")[..6], "A"); var b = await CreateWorkstreamAsync("RCB" + Guid.NewGuid().ToString("N")[..6], "B"); var source = await CreateDeskAsync("RCAS" + Guid.NewGuid().ToString("N")[..6], "Source", a.Id); var target = await CreateDeskAsync("RCBT" + Guid.NewGuid().ToString("N")[..6], "Target", b.Id); var id = await SeedPhase2FCPulseItemAsync(a, source, "P2FC-CROSS", SeedData.BootstrapAdminId);
        Assert.Equal(HttpStatusCode.OK, (await client.PostAsJsonAsync($"/api/work-items/{id}/reassign", new ReassignWorkItemApiRequest(target.Id, null, "cross", 1))).StatusCode); using var s = _factory.Services.CreateScope(); var db = s.ServiceProvider.GetRequiredService<LacDbContext>(); Assert.Equal(a.Id, (await db.WorkItems.FindAsync(id))!.WorkstreamId); Assert.Equal(target.Id, (await db.WorkItemAssignments.SingleAsync(x => x.WorkItemId == id && x.IsActive)).OfficeDeskId);
    }

    [Fact]
    public async Task Phase2FC_InstitutionalDeskMembers_CanSeeAndAct_WhileContributorCannotEstablishFirstAction()
    {
        var ws = await CreateWorkstreamAsync("RIN" + Guid.NewGuid().ToString("N")[..6], "Institution"); var desk = await CreateDeskAsync("RIND" + Guid.NewGuid().ToString("N")[..6], "Desk", ws.Id, assignAdmin: false); var (_, named) = await CreateScopedUserClientAsync("rina" + Guid.NewGuid().ToString("N")[..6], "rin-a-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Assigned, deskId: desk.Id, workstreamId: ws.Id); var (member, memberId) = await CreateScopedUserClientAsync("rinb" + Guid.NewGuid().ToString("N")[..6], "rin-b-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Assigned, deskId: desk.Id, workstreamId: ws.Id); var id = await SeedPhase2FCPulseItemAsync(ws, desk, "P2FC-INSTITUTION", SeedData.BootstrapAdminId);
        using (var s = _factory.Services.CreateScope()) { var db = s.ServiceProvider.GetRequiredService<LacDbContext>(); (await db.WorkItemAssignments.SingleAsync(x => x.WorkItemId == id && x.IsActive)).AssignedUserId = named; await db.SaveChangesAsync(); }
        Assert.Equal(HttpStatusCode.OK, (await member.PostAsync($"/api/work-items/{id}/seen", null)).StatusCode); Assert.Equal(HttpStatusCode.OK, (await member.PostAsJsonAsync($"/api/work-items/{id}/start", new StartWorkApiRequest(1))).StatusCode);
        using var check = _factory.Services.CreateScope(); var db2 = check.ServiceProvider.GetRequiredService<LacDbContext>(); var assignment = await db2.WorkItemAssignments.SingleAsync(x => x.WorkItemId == id && x.IsActive); Assert.Equal(memberId, assignment.FirstSeenByUserId); Assert.Equal(memberId, assignment.FirstActionByUserId); Assert.NotEqual(named, memberId);
    }

    [Fact]
    public async Task Phase2FC_Reassign_Timeline_UsesImmutableRoutingSnapshotsAfterRename()
    {
        var client = await CreateAdminClientAsync(); var ws = await CreateWorkstreamAsync("RTL" + Guid.NewGuid().ToString("N")[..6], "Timeline"); var source = await CreateDeskAsync("RTLS" + Guid.NewGuid().ToString("N")[..6], "Original Source", ws.Id); var target = await CreateDeskAsync("RTLT" + Guid.NewGuid().ToString("N")[..6], "Original Target"); var id = await SeedPhase2FCPulseItemAsync(ws, source, "P2FC-TIMELINE", SeedData.BootstrapAdminId);
        Assert.Equal(HttpStatusCode.OK, (await client.PostAsJsonAsync($"/api/work-items/{id}/reassign", new ReassignWorkItemApiRequest(target.Id, null, "retain reason", 1))).StatusCode); using (var s = _factory.Services.CreateScope()) { var db = s.ServiceProvider.GetRequiredService<LacDbContext>(); (await db.OfficeDesks.FindAsync(source.Id))!.Name = "Renamed Source"; (await db.OfficeDesks.FindAsync(target.Id))!.Name = "Renamed Target"; await db.SaveChangesAsync(); }
        var timeline = await client.GetStringAsync($"/api/work-items/{id}/timeline"); Assert.Contains("Original Source", timeline); Assert.Contains("Original Target", timeline); Assert.Contains("retain reason", timeline); Assert.DoesNotContain("Renamed Source", timeline); Assert.DoesNotContain("Renamed Target", timeline);
    }
    ...
    [Fact]
    public async Task Phase2FC_BranchPulse_AttentionFilters_UseExactDerivedSemantics()
    {
        var client = await CreateAdminClientAsync(); var ws = await CreateWorkstreamAsync("PAF" + Guid.NewGuid().ToString("N")[..6], "Filters"); var desk = await CreateDeskAsync("PAFD" + Guid.NewGuid().ToString("N")[..6], "Desk", ws.Id); var prefix = "P2FC-ATT-FILTER-" + Guid.NewGuid().ToString("N"); var now = DateTimeOffset.UtcNow; var india = TimeZoneInfo.FindSystemTimeZoneById("India Standard Time"); var delhi = TimeZoneInfo.ConvertTime(now, india).Date; var todayDue = TimeZoneInfo.ConvertTimeToUtc(delhi.AddHours(12), india); var ids = new Dictionary<string, Guid>();
        using (var s = _factory.Services.CreateScope()) { var db = s.ServiceProvider.GetRequiredService<LacDbContext>(); foreach (var spec in new[] { ("overdue", WorkItemStatus.InProgress, (DateTimeOffset?)now.AddHours(-1), now), ("today", WorkItemStatus.InProgress, (DateTimeOffset?)new DateTimeOffset(todayDue), now), ("review", WorkItemStatus.InProgress, (DateTimeOffset?)null, now), ("waiting", WorkItemStatus.InProgress, (DateTimeOffset?)null, now), ("returned", WorkItemStatus.InProgress, (DateTimeOffset?)null, now), ("notstarted", WorkItemStatus.Assigned, (DateTimeOffset?)null, now), ("nohandler", WorkItemStatus.InProgress, (DateTimeOffset?)null, now), ("stale", WorkItemStatus.InProgress, (DateTimeOffset?)null, now.AddDays(-9)), ("terminal", WorkItemStatus.Completed, (DateTimeOffset?)now.AddHours(-1), now.AddDays(-9)) }) { var item = new WorkItem { Id = Guid.NewGuid(), WorkstreamId = ws.Id, Title = prefix + spec.Item1, Status = spec.Item2, DueAt = spec.Item3, RequestedByUserId = SeedData.BootstrapAdminId, RequestedByDisplayNameSnapshot = "Admin", Revision = 1, LastActivityAt = spec.Item4, RecordStatus = RecordStatus.Active }; var assignment = new WorkItemAssignment { Id = Guid.NewGuid(), WorkItemId = item.Id, OfficeDeskId = desk.Id, AssignedUserId = spec.Item1 == "nohandler" ? null : SeedData.BootstrapAdminId, IsActive = true, AssignedAt = now, RecordStatus = RecordStatus.Active }; db.AddRange(item, assignment); ids[spec.Item1] = item.Id; } db.WorkItemContributors.AddRange(new WorkItemContributor { Id = Guid.NewGuid(), WorkItemId = ids["review"], UserId = SeedData.BootstrapAdminId, AddedByUserId = SeedData.BootstrapAdminId, AddedAt = now, Status = WorkItemContributorStatus.Submitted, IsActive = true, RecordStatus = RecordStatus.Active }, new WorkItemContributor { Id = Guid.NewGuid(), WorkItemId = ids["waiting"], UserId = SeedData.BootstrapAdminId, AddedByUserId = SeedData.BootstrapAdminId, AddedAt = now, Status = WorkItemContributorStatus.Active, IsActive = true, RecordStatus = RecordStatus.Active }, new WorkItemContributor { Id = Guid.NewGuid(), WorkItemId = ids["returned"], UserId = SeedData.BootstrapAdminId, AddedByUserId = SeedData.BootstrapAdminId, AddedAt = now, Status = WorkItemContributorStatus.Returned, IsActive = true, RecordStatus = RecordStatus.Active }); await db.SaveChangesAsync(); }
        async Task<HashSet<Guid>> Filter(string value) { using var doc = JsonDocument.Parse(await client.GetStringAsync($"/api/work-items/branch-pulse?q={prefix}&attention={value}&staleDays=7&pageSize=100")); return doc.RootElement.GetProperty("items").EnumerateArray().Select(x => x.GetProperty("workItemId").GetGuid()).ToHashSet(); }
        Assert.Contains(ids["overdue"], await Filter("overdue")); Assert.Contains(ids["today"], await Filter("due-today")); Assert.Contains(ids["review"], await Filter("needs-review")); Assert.DoesNotContain(ids["review"], await Filter("waiting-on-help")); var waiting = await Filter("waiting-on-help"); Assert.Contains(ids["waiting"], waiting); Assert.Contains(ids["returned"], waiting); Assert.Contains(ids["notstarted"], await Filter("not-started")); Assert.Contains(ids["nohandler"], await Filter("no-named-handler")); Assert.Contains(ids["stale"], await Filter("stale")); Assert.DoesNotContain(ids["terminal"], await Filter("open"));
    }

    [Fact]
    public async Task Phase2FC_MyWork_ReassignmentMovesDeskParticipation_PreservesContributorAndRequester()
    {
        var ws = await CreateWorkstreamAsync("PMW" + Guid.NewGuid().ToString("N")[..6], "My Work"); var oldDesk = await CreateDeskAsync("PMWO" + Guid.NewGuid().ToString("N")[..6], "Old", ws.Id, assignAdmin: false); var newDesk = await CreateDeskAsync("PMWN" + Guid.NewGuid().ToString("N")[..6], "New", ws.Id, assignAdmin: false); var (oldClient, _) = await CreateCustomUserClientAsync("pmwo" + Guid.NewGuid().ToString("N")[..6], "pmwo-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Assigned, [PermissionCodes.WorkItemView], oldDesk.Id, ws.Id); var (newClient, _) = await CreateCustomUserClientAsync("pmwn" + Guid.NewGuid().ToString("N")[..6], "pmwn-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Assigned, [PermissionCodes.WorkItemView], newDesk.Id, ws.Id); var (contributorClient, contributor) = await CreateCustomUserClientAsync("pmwc" + Guid.NewGuid().ToString("N")[..6], "pmwc-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.Assigned, [PermissionCodes.WorkItemView], workstreamId: ws.Id); var (requesterClient, requester) = await CreateCustomUserClientAsync("pmwr" + Guid.NewGuid().ToString("N")[..6], "pmwr-" + Guid.NewGuid().ToString("N")[..6], ScopeMode.All, [PermissionCodes.WorkItemView], workstreamId: ws.Id); var id = await SeedPhase2FCPulseItemAsync(ws, oldDesk, "P2FC-MYWORK-" + Guid.NewGuid().ToString("N"), requester);
        using (var s = _factory.Services.CreateScope()) { var db = s.ServiceProvider.GetRequiredService<LacDbContext>(); db.WorkItemContributors.Add(new WorkItemContributor { Id = Guid.NewGuid(), WorkItemId = id, UserId = contributor, AddedByUserId = SeedData.BootstrapAdminId, AddedAt = DateTimeOffset.UtcNow, Status = WorkItemContributorStatus.Active, IsActive = true, RecordStatus = RecordStatus.Active }); await db.SaveChangesAsync(); }
        async Task<bool> Has(HttpClient c) { using var doc = JsonDocument.Parse(await c.GetStringAsync("/api/work-items/my-work")); return doc.RootElement.GetProperty("items").EnumerateArray().Any(x => x.GetProperty("id").GetGuid() == id); }
        Assert.True(await Has(oldClient)); Assert.False(await Has(newClient)); Assert.True(await Has(contributorClient)); Assert.True(await Has(requesterClient)); var admin = await CreateAdminClientAsync(); Assert.Equal(HttpStatusCode.OK, (await admin.PostAsJsonAsync($"/api/work-items/{id}/reassign", new ReassignWorkItemApiRequest(newDesk.Id, null, "move", 1))).StatusCode); Assert.False(await Has(oldClient)); Assert.True(await Has(newClient)); Assert.True(await Has(contributorClient)); Assert.True(await Has(requesterClient));
    }
