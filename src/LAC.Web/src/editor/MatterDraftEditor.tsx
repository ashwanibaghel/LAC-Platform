...
function MatterDraftCanvas({ draft, onChange, onPrint }: { draft: Draft; onChange: (contentJson: string) => void; onPrint: () => void }) {
  ...
  return <div className="draft-editor-body"><EditorToolbar editor={editor} onPrint={onPrint} /><div className={`draft-page-wrap ${draft.orientation.toLowerCase()}`}><main className="draft-page" style={pageStyle}><EditorContent editor={editor} /></main></div></div>;
}
...
export function MatterDraftEditorPage() {
  ...
  const save = async () => { if (!draft || saving) return; const savedDraft = draft; const savedContent = contentJson; const savedGeneration = editGeneration.current; setSaving(true); setError(""); try { const result = await request<{ revision: number; updatedAt: string }>(`/matter-drafts/${savedDraft.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: savedDraft.title, contentJson: savedContent, pageSize: savedDraft.pageSize, orientation: savedDraft.orientation, marginTopMm: savedDraft.marginTopMm, marginRightMm: savedDraft.marginRightMm, marginBottomMm: savedDraft.marginBottomMm, marginLeftMm: savedDraft.marginLeftMm, expectedRevision: savedDraft.revision }) }); setDraft(current => current ? { ...current, revision: result.revision, updatedAt: result.updatedAt } : current); setDirty(editGeneration.current !== savedGeneration); } catch (e) { setError(e instanceof Error ? e.message : "Could not save draft."); } finally { setSaving(false); } };
  ...
  const print = () => { let style = document.getElementById("matter-draft-print-layout") as HTMLStyleElement | null; if (!style) { style = document.createElement("style"); style.id = "matter-draft-print-layout"; document.head.append(style); } style.textContent = `@media print { @page { size: A4 ${draft.orientation.toLowerCase()}; margin: 0; } }`; window.print(); };
