...
export const AccessAdmin: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"roles" | "designations" | "workstreams">("roles");
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const [rolesRes, permsRes, desigRes, wsRes] = await Promise.all([
      ...
      if (!rolesRes.ok || !permsRes.ok || !desigRes.ok || !wsRes.ok) {
  ...
  const openEditRoleModal = (r: RoleDetail) => {
    setEditingRole(r);
    setRoleCode(r.code);
    setRoleName(r.name);
    ...
    <div className="admin-page">
      <div className="section-heading">
        <div>
          ...
          <span>Designations $\neq$ Roles. Roles bundle software authorities with granular scopes.</span>
        </div>
        {activeTab === "roles" && (
            ...
            <tbody>
              {roles.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.description || "—"}</td>
                  <td>
                    <span className={`status ${r.isSystemRole ? "warning" : "success"}`}>
                  ...
                  <td>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                      {r.permissions.map((p) => (
                        <span key={p.code} className="metric-badge" style={{ fontSize: "11px", padding: "2px 6px", background: "#edf5fc", border: "1px solid #c9dfef", borderRadius: "4px" }}>
                          {p.code} <small style={{ color: "#1c5d9f", fontWeight: 700 }}>[{p.scopeMode}]</small>
                        </span>
                      ))}
                    </div>
                  </td>
                  <td>
                    <button className="quiet-button text-action" onClick={() => openEditRoleModal(r)}>
                      Configure
                    </button>
                  </td>
                </tr>
        ...
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Code</th>
                <th>Official Civil Post Name</th>
              </tr>
            </thead>
            <tbody>
              {designations.map((d) => (
