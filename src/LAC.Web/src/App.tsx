...
function useApi<T>(path?: string): State<T> {
  ...
  useEffect(() => {
    ...
    fetch(api + path, { signal: controller.signal })
      ...
      .catch((error) => {
        // A route/filter change aborts the previous request. It is expected
        // cleanup, not a visible data-service failure.
        if (active && error?.name !== "AbortError" && cached === undefined) {
...
async function uploadNmPdf<T>(awardId:string,file:File):Promise<T>{const form=new FormData();form.append("file",file);const response=await fetch(`${api}/awards/${awardId}/nm-documents`,{method:"POST",body:form});if(!response.ok)throw new Error((await response.json().catch(()=>null))?.detail||"Could not store the NM PDF.");clearApiCache();return response.json() as Promise<T>;}
...
function Matter(){const {id=""}=useParams();const [refresh,setRefresh]=useState(0);const [selected,setSelected]=useState<string[]>([]);const [file,setFile]=useState<File>();const [role,setRole]=useState("Application");const data=useApi<any>(`/matters/${id}?r=${refresh}`);const docs=useApi<any[]>(`/matters/${id}/documents?r=${refresh}`);const toggle=(docId:string)=>setSelected(x=>x.includes(docId)?x.filter(v=>v!==docId):[...x,docId]);const upload=async()=>{if(!file)return;const form=new FormData();form.append("file",file);try{await fetch(`${api}/matters/${id}/documents?role=${encodeURIComponent(role)}`,{method:"POST",body:form}).then(async r=>{if(!r.ok)throw new Error(await r.text())});setFile(undefined);setRefresh(x=>x+1)}catch{}};const exportZip=async()=>{const response=await fetch(`${api}/matters/${id}/export`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({documentIds:selected})});if(!response.ok)return;const url=URL.createObjectURL(await response.blob());const link=document.createElement("a");link.href=url;link.download="matter-documents.zip";link.click();URL.revokeObjectURL(url)};if(data.loading)return <LoadingState/>;if(data.error||!data.data)return <ErrorState message={data.error||"Matter not found."}/>;const m=data.data;return <><Breadcrumbs items={[{label:"Village",to:route.village(m.villageId)},{label:m.title}]}/><PageHeader eyebrow="Matter workspace" title={m.title}><p>{m.matterType} · {m.status} · {m.khasraReferenceText||"No Khasra reference"}</p></PageHeader><section className="section"><h2>Core Records — {m.award?.awardNumber||"No Award selected"}</h2>{m.award?.documents?.length?m.award.documents.map((d:any)=><p key={d.documentId}><input type="checkbox" checked={selected.includes(d.documentId)} onChange={()=>toggle(d.documentId)}/> <strong>{d.role}</strong> · <a href={`${api}/documents/${d.documentId}/content`} target="_blank" rel="noreferrer">{d.originalFileName}</a></p>):<p>No Award core documents are available.</p>}</section><section className="section"><h2>Matter Documents</h2><div className="field-grid"><select value={role} onChange={e=>setRole(e.target.value)}>{["Application","Court Order","ADM Letter","Joint Declaration","Khatoni","Demarcation","Correspondence","Other"].map(x=><option key={x}>{x}</option>)}</select><input type="file" onChange={e=>setFile(e.target.files?.[0])}/><button disabled={!file} onClick={()=>void upload()}>Upload Document</button><button disabled={!selected.length} onClick={()=>void exportZip()}>Export Selected ({selected.length})</button></div>{docs.data?.map(d=><p key={d.documentId}><input type="checkbox" checked={selected.includes(d.documentId)} onChange={()=>toggle(d.documentId)}/> <strong>{d.documentRole||"Other"}</strong> · <a href={`${api}/documents/${d.documentId}/content`} target="_blank" rel="noreferrer">{d.displayName||d.originalFileName}</a></p>)}<p className="hint">Drafts: letters and notings will appear here in the next phase.</p></section></>}
...
function Award() {
  ...
  return (
    <>
      ...
      {(a.parentAwardNumber || a.parentAwardReference) && <section className="supplementary-reference"><strong>Main Award reference</strong><span>{a.parentAwardNumber || a.parentAwardReference}</span>{a.parentAwardNumber && a.parentAwardReference && a.parentAwardReference !== a.parentAwardNumber && <small>Source reference: {a.parentAwardReference}</small>}{!a.parentAwardNumber && <small>Not linked to a main Award record yet; this reference remains for review.</small>}</section>}
...
function AwardPdfImportPanel({ award, onClose, onUploaded }: { award?: any; onClose?: () => void; onUploaded?: () => void }) {
  const [file,setFile]=useState<File>();const [message,setMessage]=useState("");const [busy,setBusy]=useState(false);const [uploaded,setUploaded]=useState<any>();const [selectedAwardId,setSelectedAwardId]=useState("");const [createdAward,setCreatedAward]=useState<any>();const [unlinkedRefresh,setUnlinkedRefresh]=useState(0);const [supplementary,setSupplementary]=useState({awardNumber:"",awardDate:"",parentAwardReference:""});
  ...
  const selectedAward=award||createdAward||awardChoices.data?.items.find(item=>item.id===selectedAwardId);const selectedVillageId=selectedAward?.villages?.length===1?selectedAward.villages[0].id:undefined;
  ...
  const createSupplementary=async()=>{try{if(!supplementary.awardNumber.trim()||!supplementary.parentAwardReference.trim())throw new Error("Supplementary Award number and main Award reference are required.");setBusy(true);const result:any=await post("/awards/supplementary",{awardNumber:supplementary.awardNumber,awardDate:supplementary.awardDate||null,parentAwardReference:supplementary.parentAwardReference});setCreatedAward({id:result.id,awardNumber:supplementary.awardNumber});setSelectedAwardId(result.id);setMessage("Supplementary Award created. It is not auto-linked to a guessed main Award; now link the stored PDF explicitly.");}catch(error){setMessage(error instanceof Error?error.message:"Could not create the Supplementary Award.");}finally{setBusy(false);}};
  const body=<section className="workspace-panel"><div className="panel-title"><h3>{award?"Upload Award PDF":"Link an Award PDF"}</h3>{onClose&&<button className="quiet-button" onClick={onClose}>Close</button>}</div><p>{award?"This PDF will be stored and linked to the Award shown below. Analysis remains your explicit next step.":"Normal uploads begin inside an Award workspace. This recovery page requires an Award before it can store or link a PDF."}</p><div className="field-grid">{award?<label>Award<input readOnly value={award.awardNumber} /></label>:<label className="span-two">Award<select value={selectedAwardId} onChange={e=>{setCreatedAward(undefined);setSelectedAwardId(e.target.value)}}><option value="">Select the Award that owns this PDF</option>{createdAward&&<option value={createdAward.id}>{createdAward.awardNumber} · Supplementary (new)</option>}{awardChoices.data?.items.map(item=><option key={item.id} value={item.id}>{item.awardNumber}{item.awardDate?` · ${date(item.awardDate)}`:""}</option>)}</select></label>}<label className="span-two">PDF file<input disabled={!selectedAward||Boolean(uploaded)} type="file" accept="application/pdf,.pdf" onChange={e=>setFile(e.target.files?.[0])}/></label></div><div className="form-footer"><span className="hint">Upload only stores and links the PDF. It does not start OCR or write Award facts.</span><button disabled={busy||!selectedAward||!file||Boolean(uploaded)} onClick={submit}>{busy?"Working…":"Upload PDF"}</button></div>{uploaded&&<div className="upload-success"><strong>PDF uploaded and linked to this Award.</strong><span>It is ready to view or analyze whenever you choose.</span><div><a className="secondary-button" href={`${api}/documents/${uploaded.documentId}/content`} target="_blank" rel="noreferrer">View PDF</a><button disabled={busy} onClick={analyze}>Analyze Data</button></div></div>}{!award&&<details className="supplementary-create"><summary>Create a Supplementary Award</summary><p>Use this when the document is a separate supplementary Award. The main Award reference is saved as text unless you explicitly link a real main Award later.</p><div className="field-grid"><label>Award number<input value={supplementary.awardNumber} onChange={e=>setSupplementary(x=>({...x,awardNumber:e.target.value}))} placeholder="e.g. 2-A (Supplementary)/2018/NHAI"/></label><label>Award date<input type="date" value={supplementary.awardDate} onChange={e=>setSupplementary(x=>({...x,awardDate:e.target.value}))}/></label><label className="span-two">Main Award reference<input value={supplementary.parentAwardReference} onChange={e=>setSupplementary(x=>({...x,parentAwardReference:e.target.value}))} placeholder="e.g. 02/2018/NHAI dated 18.12.2018"/></label></div><div className="form-footer"><span className="hint">Creating this Award does not create the main Award, start OCR, or add Khasras/notifications/claims.</span><button disabled={busy} onClick={createSupplementary}>Create Supplementary Award</button></div></details>}{!award&&unlinked.data?.length? <section className="stored-pdf-recovery"><h4>Previously stored PDFs needing an Award link</h4><p>Select the owning Award above, then link an existing file without uploading it again.</p>{unlinked.data.map(doc=><div key={doc.documentId}><span><strong>{doc.originalFileName}</strong><small>Stored {date(doc.uploadedAt)}</small></span><button disabled={busy||!selectedAward} onClick={()=>linkStored(doc.documentId)}>Link to selected Award</button></div>)}</section>:null}{message&&<p className="form-message">{message}</p>}</section>;
...
function AwardIngestionReview() {
  ...
  const reason = (c:any) => {try {const issue=JSON.parse(c.validationIssuesJson||"[]");if(Array.isArray(issue)&&issue.length)return issue.join(" ");const e=JSON.parse(c.sourceLocatorJson||"{}");return (e.Warnings||e.warnings||[]).join(" ") || e.Reason || e.reason || "Check source and confirm.";}catch{return "Check source and confirm.";}};
  const reviewLabel = (c:any) => {try {const details=JSON.parse(c.conflictDetailsJson||"{}");if(details.code==="RepeatedSameValues")return "Repeated in source";if(details.code==="RepeatedDifferentAreas")return "Same Khasra · multiple areas";}catch{/* normal review state */}return c.verifiedAt?`Verified by ${c.verifiedBy}`:c.safeToConfirm?"Exact match":c.status==="Conflict"?"Master conflict":c.status==="Invalid"?"Could not read":"Needs attention";};
  ...
  return <div className="review-workspace">
    ...
    {(!s.targetAwardId || !s.selectedVillageId) && <ReviewContextPicker sessionId={sessionId} reviewer={reviewer} onReviewer={setReviewer} onSaved={changed}/>}
...
function FactVerificationDrawer({candidate,documentId,awardId,reviewer,reason,onClose,onSaved}:{candidate:any;documentId:string;awardId:string;reviewer:string;reason:string;onClose:()=>void;onSaved:()=>void}) {
  ...
  const [message,setMessage]=useState("");const [busy,setBusy]=useState(false);const [officer,setOfficer]=useState(reviewer||sessionStorage.getItem("lac.reviewOfficer")||"");const [activeRole,setActiveRole]=useState("Khasra");const [fieldDecisions,setFieldDecisions]=useState<any[]>(()=>{try{return JSON.parse(candidate.fieldReviewJson||"[]");}catch{return [];}});const context=useApi<any>(awardId?`/awards/${awardId}/workspace`:undefined);
  let locator:any={};try{locator=JSON.parse(candidate.sourceLocatorJson||"{}");}catch{/* No guessed source. */}
  let occurrence:any={};try{occurrence=JSON.parse(candidate.conflictDetailsJson||"{}");}catch{/* Ordinary source occurrence. */}
  ...
  const sourceKey=activeRole==="Khasra"||activeRole==="Qualifier"?"khasra":activeRole==="RecordedArea"?"recordedArea":"awardedArea";
  const sourceCell=locator.StructuredPayload?.sourceCells?.[sourceKey] || locator.structuredPayload?.sourceCells?.[sourceKey];
  const crop=candidate.candidateType==="AwardKhasra"?`${api}/award-ingestion-candidates/${candidate.id}/source-crop?fieldRole=${encodeURIComponent(sourceKey)}`:undefined;
  ...
  useEffect(()=>{const keys=(event:KeyboardEvent)=>{const tag=(event.target as HTMLElement)?.tagName;if(event.key==="Escape")onClose();if(["INPUT","TEXTAREA","SELECT"].includes(tag))return;if(!busy&&!candidate.verifiedAt&&officer.trim()&&event.key.toLowerCase()==="s"){event.preventDefault();void save("Skip");}if(!busy&&!candidate.verifiedAt&&officer.trim()&&event.key.toLowerCase()==="u"){event.preventDefault();void save("Uncertain");}};window.addEventListener("keydown",keys);return()=>window.removeEventListener("keydown",keys);},[onClose,busy,candidate.verifiedAt,officer]);
  const field=(key:string,label:string,kind:"text"|"number"|"date"="text",role?:string)=><label key={key}>{label}<input type={kind} step={kind==="number"?"any":undefined} value={value[key]??""} onFocus={()=>role&&setActiveRole(role)} onChange={e=>setValue({...value,[key]:e.target.value===""?null:kind==="number"?Number(e.target.value):e.target.value})}/></label>;
  const area=(prefix:string,label:string,role:string)=><fieldset><legend>{label}</legend><div className="field-grid">{field(prefix+"Bigha","Bigha","number",role)}{field(prefix+"Biswa","Biswa","number",role)}{field(prefix+"Biswansi","Biswansi","number",role)}</div></fieldset>;
  const fieldValue=()=>activeRole==="Khasra"?String(value.khasraNumber||""):activeRole==="Qualifier"?String(value.qualifier||""):`${activeRole==="RecordedArea"?value.recordedAreaBigha:value.awardedAreaBigha}-${activeRole==="RecordedArea"?value.recordedAreaBiswa:value.awardedAreaBiswa}${(activeRole==="RecordedArea"?value.recordedAreaBiswansi:value.awardedAreaBiswansi)===null||((activeRole==="RecordedArea"?value.recordedAreaBiswansi:value.awardedAreaBiswansi)===undefined)?"":`-${activeRole==="RecordedArea"?value.recordedAreaBiswansi:value.awardedAreaBiswansi}`}`;
  const requiredRoles=()=>{const cells=locator.StructuredPayload?.sourceCells||locator.structuredPayload?.sourceCells||{};const roles=["Khasra",...(cells.recordedArea?["RecordedArea"]:[]),...(cells.awardedArea?["AwardedArea"]:[]),...(value.qualifier?["Qualifier"]:[])];return roles;};
  const save=async(action:string)=>{setBusy(true);setMessage("");try{if(candidate.candidateType==="AwardKhasra"){const suggested=activeRole==="Qualifier"?value.qualifier:sourceCell?.normalizedSuggestion;const human=action==="Confirm"?fieldValue():null;const decision=action==="Confirm"?(human===suggested?"Confirm":"Correct"):action;await post(`/award-ingestion-candidates/${candidate.id}/verify-award-khasra-field`,{verifiedBy:officer,fieldRole:activeRole,humanValue:human,decision});const next=[...fieldDecisions.filter(d=>d.fieldRole!==activeRole),{fieldRole:activeRole,decision,humanValue:human}];setFieldDecisions(next);const pending=requiredRoles().find(role=>!next.some(d=>d.fieldRole===role));if(pending){setActiveRole(pending);setMessage(`${activeRole} saved. Continue with ${pending}.`);}else onSaved();}else{await post(`/award-ingestion-candidates/${candidate.id}/verify`,{verifiedBy:officer,correctedPayloadJson:JSON.stringify(value),action});onSaved();}}catch(e){setMessage(e instanceof Error?e.message:"Could not verify.");}finally{setBusy(false);}};
  ...
  return <aside className="evidence-workbench" role="dialog" aria-modal="true" aria-label="Verify source fact"><form className="evidence-editor" onSubmit={e=>{e.preventDefault();if(supported&&!busy&&awardId&&officer.trim()&&!candidate.verifiedAt)void save("Confirm");}}>
    <div className="panel-title"><div><span className="review-step">Review source fact</span><h2>{reviewSectionName(candidate.candidateType)}</h2></div><button type="button" onClick={onClose}>← Back to review</button></div><div className="review-guide"><strong>What to do here</strong><p>{guide}</p></div><p className="review-reason"><strong>Why it needs review:</strong> {reason}</p>{occurrence.code&&<section className="source-occurrences"><strong>{occurrence.code==="RepeatedSameValues"?"Repeated in source":"Same Khasra has multiple area entries"}</strong><p>Every occurrence remains separate. Confirm what this PDF row says; do not merge or add areas here.</p>{occurrence.occurrences?.map((item:any,index:number)=><div key={item.id}><span>Occurrence {index+1} · Page {item.sourcePage||"?"} · Recorded {item.recordedArea||"—"} · Awarded {item.awardedArea||"—"}</span><a href={`${api}/award-ingestion-candidates/${item.id}/source-crop?fieldRole=khasra`} target="_blank" rel="noreferrer">View exact crop</a></div>)}</section>}{!awardId&&<p className="review-blocker">Choose the Award on the review page first to enable confirmation.</p>}{(locator.PossibleCanonicalMatch||locator.possibleCanonicalMatch)&&<p>Possible master: {locator.PossibleCanonicalMatch||locator.possibleCanonicalMatch}. Enter that identifier yourself to link it; no digit is changed automatically.</p>}
    {candidate.candidateType==="AwardCore"?<><div className="field-grid">{field("awardNumber","Award number")}{field("awardDate","Award date","date")}{field("awardType","Award type")}{field("natureOfAcquisition","Nature of acquisition")}{field("awardedAreaText","Awarded / acquisition area")}{field("parentAwardReferenceSuggestion","Parent/Main Award reference")}</div><label>Purpose of acquisition<textarea value={value.purpose||""} onChange={e=>setValue({...value,purpose:e.target.value||null})}/></label><p className="hint">Parent Award reference is evidence only; it never links an Award automatically. Unreadable identifiers are never guessed.</p></>:candidate.candidateType==="AwardVillage"?<><label>Official Village<select value={value.villageName||""} onChange={e=>setValue({...value,villageName:e.target.value,exactCanonicalVillageName:e.target.value})}><option value="">Select the official spelling</option>{context.data?.villages?.map((v:any)=><option value={v.name} key={v.id}>{v.name}</option>)}</select></label><p className="hint">Detected: {JSON.parse(candidate.payloadJson).villageName}. Confirm the official Village linked to this Award.</p></>:candidate.candidateType==="AwardKhasra"?<><div className="field-decision-status">{requiredRoles().map(role=><span key={role} className={role===activeRole?"active":""}>{role}: {fieldDecisions.find(d=>d.fieldRole===role)?.decision||"Pending"}</span>)}</div><div className="field-grid">{field("khasraNumber","Khasra number","text","Khasra")}{value.qualifier!==null&&field("qualifier","Qualifier","text","Qualifier")}</div>{area("recordedArea","Total Area Recorded in Award","RecordedArea")}{area("awardedArea","Area Awarded in this Award","AwardedArea")}<p className="hint">Each field is saved separately. This row is only verified after every source-backed required field has a human confirmation or correction.</p></>:candidate.candidateType==="Notification"?<div className="field-grid">{field("sectionType","Legal framework / section")}{field("notificationNumber","Notification number")}{field("notificationDate","Notification date","date")}</div>:candidate.candidateType==="PossessionEvent"?<><div className="field-grid">{field("possessionDate","Possession date","date")}{field("eventType","Normalized event type")}{field("status","Exact source status wording")}{field("possessionAreaText","Source possession area")}{field("possessionAreaUnit","Source area unit")}{field("khasraReferences","Source Khasra references")}</div><p>Possession area is independent from Award and Village areas. Khasras here are source references only: no AwardKhasra or possession link is inferred. Confirming this creates no canonical record until the separate Commit action.</p></>:candidate.candidateType==="CourtCase"?<><div className="field-grid">{field("caseNumber","Case number / identifier")}{field("caseType","Case type")}{field("courtName","Court (only if stated)")}{field("status","Exact status / order wording")}{field("khasraReferences","Source Khasra references")}{field("relatedAreaText","Source area reference")}{field("parties","Source parties (only if stated)")}</div><p><strong>Case reference and legal effect are separate.</strong> A case or CWP does not by itself mean a stay, release, payment block, or affected-parcel relationship. Khasra/area values stay source references until a later human workflow resolves them.</p></>:candidate.candidateType==="Claim"?<><div className="field-grid">{field("claimReference","Claim reference")}{field("claimDate","Claim date","date")}</div><label>Claim text<textarea value={value.claimText||""} onChange={e=>setValue({...value,claimText:e.target.value})}/></label><p>Names do not identify or merge Parties automatically.</p></>:candidate.candidateType==="AwardLandClass"?<div className="field-grid">{field("code","Class code")}{field("description","Description")}</div>:candidate.candidateType==="AwardValuationRule"?<div className="field-grid">{field("ruleType","Valuation rule")}{field("rateAmount","Rate amount","number")}{field("rateUnit","Rate unit")}{field("legalSection","Legal section")}</div>:candidate.candidateType==="AwardCompensationRule"?<div className="field-grid">{field("ruleType","Compensation rule")}{field("ratePercent","Rate percent","number")}{field("rateAmount","Amount","number")}{field("legalSection","Legal section")}</div>:candidate.candidateType==="AwardAreaIssue"?<div className="field-grid">{field("issueType","Issue")}{field("notificationAreaBigha","Notification area (Bigha)","number")}{field("fieldBookAreaBigha","Field-book area (Bigha)","number")}{field("differenceBigha","Difference (Bigha)","number")}</div>:candidate.candidateType==="AwardSupplementaryMatter"?<div className="field-grid">{field("matterType","Matter")}{field("description","Description")}</div>:<p>{value.summary||"This is source evidence, not a structured record ready for confirmation."}</p>}
    <section className="source-spotlight"><div><span>Source match · Page {page||"not identified"}</span><strong>{candidate.candidateType==="AwardKhasra"?activeRole:"evidence"}</strong></div>{crop&&<img className="source-crop" src={crop} alt={`Exact ${activeRole} cell from page ${page}`}/>}<mark>{sourceCell?.rawOcr||candidate.rawSourceText||value.extractedText||"No readable text was returned; inspect the page manually."}</mark>{sourceCell?.normalizedSuggestion&&<small>Suggested value: <strong>{sourceCell.normalizedSuggestion}</strong>{sourceCell.normalizationReason?` · ${sourceCell.normalizationReason}`:""}</small>}{sourceCell?.pageAssignedOcr&&sourceCell?.cellCropOcr&&sourceCell.pageAssignedOcr!==sourceCell.cellCropOcr&&<small className="ocr-warning">OCR disagreement: page and crop readings differ. Verify visually.</small>}<small>The full PDF at right remains the primary evidence.</small></section><details open><summary>Original extracted text</summary><p style={{whiteSpace:"pre-wrap",overflowWrap:"anywhere"}}>{sourceCell?.rawOcr||candidate.rawSourceText||value.extractedText||"No readable text."}</p></details>
    {message&&<p role="alert" className="form-message">{message}</p>}
    <div className="form-footer"><label className="review-officer">Verifying officer<input value={officer} onChange={e=>{setOfficer(e.target.value);sessionStorage.setItem("lac.reviewOfficer",e.target.value);}} placeholder="Your name"/></label>{!candidate.verifiedAt&&<><span className="review-action-hint">Enter confirms. S skips; U leaves it uncertain. Confirmation is saved, but canonical records are changed only by the separate Commit action.</span>{supported&&<button type="submit" disabled={busy||!awardId||!officer.trim()}>{busy?"Saving…":"Confirm & next →"}</button>}{candidate.candidateType==="AwardKhasra"&&<button type="button" disabled={busy||!awardId||!officer.trim()} onClick={()=>save("LinkExisting")}>Link existing</button>}<button type="button" disabled={busy||!officer.trim()} onClick={()=>save("Uncertain")}>Uncertain</button><button type="button" disabled={busy||!officer.trim()} onClick={()=>save("Skip")}>Skip</button></>}</div>
...
function NmSemanticReviewWorkspaceV2(){
  void NmSemanticReviewWorkspace;
  const {id:awardId="",nmId=""}=useParams(); const [refresh,setRefresh]=useState(0); const [filter,setFilter]=useState<"unresolved"|"resolved"|"all">("unresolved"); const [fieldFilter,setFieldFilter]=useState<"all"|"area"|"khasra"|"other">("all"); const [activeIndex,setActiveIndex]=useState(0); const [reviewer,setReviewer]=useState(()=>sessionStorage.getItem("lac.reviewOfficer")||""); const [areaValue,setAreaValue]=useState(""); const [message,setMessage]=useState(""); const [saving,setSaving]=useState(false);
  const latest=useApi<any>(`/nm-documents/${nmId}/semantic-sessions/latest?r=${refresh}`); const workspace=useApi<any>(latest.data?`/nm-semantic-sessions/${latest.data.id}/review-workspace?r=${refresh}`:undefined);
  const label=(item:any)=>item.reason==="ParcelAreaMismatch"?"Area could not be safely read from NM":item.reason==="IncompleteKhasra"?"Khasra is incomplete in source":item.reason==="KhasraNotInVillageMaster"?"Khasra has no exact village-master match":item.reason==="QualifierConflict"?"Khasra qualifier needs review":item.reason==="AmbiguousDittoScope"?"Ditto relationship is ambiguous":item.reason==="PageContinuationAmbiguous"?"Page continuation is ambiguous":item.fieldName==="Share"||item.fieldName==="OwnerOrShare"?"Owner or share source evidence is missing":item.fieldName==="Area"?"Area source evidence is missing":"Required source evidence is missing";
  const queue=workspace.data?(filter==="resolved"?workspace.data.resolved:filter==="all"?[...workspace.data.queue,...workspace.data.resolved]:workspace.data.queue).filter((item:any)=>fieldFilter==="all"||(fieldFilter==="area"?item.fieldName==="Area":fieldFilter==="khasra"?item.fieldName==="Khasra":["Area","Khasra"].includes(item.fieldName)?false:true)):[]; const selected=queue[Math.min(activeIndex,Math.max(0,queue.length-1))]; const owner=selected&&workspace.data?.owners.find((item:any)=>item.id===selected.ownerBlockId); const parcel=selected?.parcel; const sourcePage=selected?.sourcePage||owner?.pageStart||1;
  useEffect(()=>{setActiveIndex(0);setAreaValue("");},[filter,fieldFilter,workspace.data?.id]); useEffect(()=>{setAreaValue("");},[selected?.id]);
  const move=(offset:number)=>{setActiveIndex(index=>Math.max(0,Math.min(queue.length-1,index+offset)));setMessage("");};
  const saveArea=async(unreadable=false)=>{if(!parcel?.id||!reviewer.trim())return;try{setSaving(true);setMessage("");if(unreadable)await post(`/nm-semantic-parcels/${parcel.id}/area/unreadable`,{reviewedBy:reviewer});else await put(`/nm-semantic-parcels/${parcel.id}/area`,{rawReviewerValue:areaValue,reviewedBy:reviewer});sessionStorage.setItem("lac.reviewOfficer",reviewer);setMessage(unreadable?"Marked source unreadable in semantic staging.":"Area saved in semantic staging. Moving to the next exception.");setRefresh(value=>value+1);setActiveIndex(index=>Math.min(index,Math.max(0,queue.length-2)));}catch(error){setMessage(error instanceof Error?error.message:"Could not save this semantic field.");}finally{setSaving(false);}};
  if(latest.loading||workspace.loading)return <LoadingState label="Loading semantic exception review…"/>; if(latest.error||workspace.error)return <ErrorState message={latest.error||workspace.error||"Semantic review is unavailable."}/>; if(!workspace.data)return <EmptyState title="No semantic session" detail="Run a limited semantic analysis before opening exception review."/>;
  const total=workspace.data.totalExceptions||0; const resolved=workspace.data.resolvedCount||0;
  return <main className="nm-semantic-workspace"><header className="nm-semantic-toolbar"><Breadcrumbs items={[{label:"Awards",to:"/awards"},{label:"Award",to:`/awards/${awardId}`},{label:"NM"},{label:"Exception Review"}]}/><div className="nm-semantic-summary"><span>{workspace.data.ownerCount} owners</span><span>{total} exceptions</span><span>{workspace.data.owners.filter((item:any)=>item.status==="ReadyForCanonicalReview").length} ready</span><span>Resolved {resolved} / {total}</span></div><label className="nm-reviewer">Reviewer<input value={reviewer} onChange={event=>setReviewer(event.target.value)} placeholder="Your name"/></label></header><div className="nm-semantic-grid"><aside className="nm-exception-queue"><div className="nm-queue-head"><strong>Review exceptions</strong><span>Unresolved {workspace.data.queue.length} · Resolved {resolved}</span></div><div className="nm-queue-filters"><div>{(["unresolved","resolved","all"] as const).map(value=><button key={value} className={filter===value?"active":""} onClick={()=>setFilter(value)}>{value}</button>)}</div><select value={fieldFilter} onChange={event=>setFieldFilter(event.target.value as any)}><option value="all">All fields</option><option value="area">Area</option><option value="khasra">Khasra</option><option value="other">Other</option></select></div><div className="nm-queue-list">{queue.map((item:any,index:number)=><button key={`${item.id||"resolved"}-${index}`} className={`nm-queue-item ${index===activeIndex?"active":""}`} onClick={()=>setActiveIndex(index)}><strong>{item.recordedNameRaw||"Recorded person unavailable"}</strong><span>{label(item)}</span><small>{item.parcel?.rawKhasraText?`Khasra ${item.parcel.rawKhasraText} · `:""}Page {item.sourcePage}</small><em>{item.parcel?.areaFieldState==="SourceUnreadable"?"Unreadable":item.parcel?.areaFieldState==="ResolvedByReviewer"?"Resolved":"Unresolved"}</em></button>)}{queue.length===0&&<EmptyState title="No matching exceptions" detail="Change the filter to view other semantic fields."/>}</div></aside><section className="nm-owner-card">{selected&&owner?<><div className="nm-owner-heading"><span>Exception {activeIndex+1} of {queue.length}</span><h1>{owner.recordedNameRaw||"Recorded person unavailable"}</h1><p>{owner.fatherOrSpouseRaw||"Father/spouse unavailable"} · {owner.residenceRaw||"Residence unavailable"} · Share {owner.shareRaw||"unreadable"}</p></div><section><h2>Parcels</h2>{owner.parcels.map((item:any)=><div className={`nm-parcel ${item.id===parcel?.id?"selected":""}`} key={item.id}><strong>{item.rawKhasraText||"Khasra unreadable"}</strong><span>Area: {item.areaReviewerValueNormalized||item.normalizedAreaText||"unresolved"}</span><span>Land class: {item.landClassRaw||"—"}</span><small>{item.validationState==="ExactSourceMasterMatch"?"✓ Exact village match":item.validationState}</small><small>{item.isInherited?"Inherited source":"Explicit source"}</small></div>)}</section><section className="nm-task-card"><span className="nm-task-kicker">{selected.fieldName||"SOURCE"} NEEDS REVIEW</span><h2>{label(selected)}</h2>{parcel&&<><p><strong>Khasra</strong><br/>{parcel.rawKhasraText||"Source Khasra unavailable"}</p>{selected.fieldName==="Area"?<><label>NM Area<input value={areaValue} onChange={event=>setAreaValue(event.target.value)} placeholder="Bigha-Biswa, e.g. 3-5" onKeyDown={event=>{if((event.key==="Enter"||event.key==="NumpadEnter")&&(event.ctrlKey||event.metaKey)){event.preventDefault();void saveArea();}}}/></label><p className="nm-secondary">Raw OCR: {parcel.rawAreaText||"No Area OCR was safely extracted."}</p>{parcel.master&&<p className="nm-master-ref">Village master reference: {parcel.master.areaBigha!=null?`${parcel.master.areaBigha}-${parcel.master.areaBiswa??0}`:"No area recorded"}<br/><small>Reference only — it is never copied into NM Area.</small></p>}<div className="nm-task-actions"><button disabled={saving||!reviewer.trim()||!areaValue.trim()} onClick={()=>void saveArea()}>{saving?"Saving…":"Save & Next"}</button><button className="quiet-button" disabled={saving||!reviewer.trim()} onClick={()=>void saveArea(true)}>Mark unreadable</button><button className="quiet-button" onClick={()=>move(1)}>Leave unresolved</button></div></>:<><p><strong>Source text</strong><br/>{parcel.rawKhasraText||"No complete Khasra identity was extracted."}</p><p className="nm-secondary">This semantic Khasra is not auto-repaired or fuzzy-matched. A correction workflow is intentionally unavailable until the officer supplies an explicit source-backed value.</p></>}</>}</section><div className="nm-nav"><button className="quiet-button" disabled={activeIndex===0} onClick={()=>move(-1)}>Previous</button><button className="quiet-button" disabled={activeIndex>=queue.length-1} onClick={()=>move(1)}>Next exception</button></div>{message&&<p className="form-message">{message}</p>}</>:<EmptyState title="No semantic exception selected" detail="Choose an exception from the queue."/>}</section><aside className="nm-semantic-source"><header><div><strong>Original NM · page {sourcePage}</strong><span>{selected?.sourceRegionJson?"Source region available":"Owner-level source region available"}</span></div></header>{workspace.data.nmDocumentId&&<DocumentPdfViewer key={`${workspace.data.nmDocumentId}-${sourcePage}`} documentId={workspace.data.nmDocumentId} initialPage={sourcePage}/>}</aside></div></main>;
}

function NmOwnerReviewWorkspace(){
  const {nmId=""}=useParams(); const [refresh,setRefresh]=useState(0); const [active,setActive]=useState(""); const [reviewer,setReviewer]=useState(""); const [draft,setDraft]=useState<any>(); const [saving,setSaving]=useState(false);
  const data=useApi<any>(`/nm-semantic-sessions/${nmId}/review-workspace?r=${refresh}`); const owners=data.data?.owners||[]; const owner=owners.find((x:any)=>x.id===active)||owners[0];
  const amount=(value:any)=>new Intl.NumberFormat("en-IN",{style:"currency",currency:"INR",minimumFractionDigits:2,maximumFractionDigits:2}).format(Number(value));
  useEffect(()=>{if(owner){setActive(owner.id);setDraft({name:owner.reviewerName??owner.recordedNameRaw??"",father:owner.reviewerFatherOrSpouse??owner.fatherOrSpouseRaw??"",residence:owner.reviewerResidence??owner.residenceRaw??"",share:owner.reviewerShare??owner.shareRaw??"",parcels:owner.parcels.map((p:any)=>({id:p.id,khasra:p.reviewerKhasra??p.rawKhasraText??"",area:p.areaReviewerValueRaw??p.rawAreaText??"",landClass:p.reviewerLandClass??p.landClassRaw??"",source:p}))});}},[owner?.id,refresh]);
  const save=async(unclear=false)=>{if(!owner||!draft||!reviewer.trim())return;setSaving(true);try{await put(`/nm-semantic-owners/${owner.id}/review`,{reviewedBy:reviewer,name:draft.name,fatherOrSpouse:draft.father,residence:draft.residence,share:draft.share,sourceUnclear:unclear,parcels:draft.parcels.map((p:any)=>({id:p.id,khasra:p.khasra,area:p.area,landClass:p.landClass}))});const i=owners.findIndex((x:any)=>x.id===owner.id);setRefresh(x=>x+1);setActive(owners[Math.min(i+1,owners.length-1)]?.id||owner.id);}finally{setSaving(false)}};
  if(data.loading)return <LoadingState/>;
  const dittoAmbiguous=owner?.exceptions?.some((x:any)=>x.reason==="AmbiguousDittoScope");
  return <main className="nm-owner-review"><header><Breadcrumbs items={[{label:"Awards",to:"/awards"},{label:"NM Owner Review"}]}/><div className="nm-owner-progress"><strong>{owners.length} owners</strong><span>{owners.filter((x:any)=>x.reviewState==="Reviewed").length} reviewed</span><span>{owners.filter((x:any)=>x.reviewState!=="Reviewed").length} remaining</span><label>Reviewer<input value={reviewer} onChange={e=>setReviewer(e.target.value)} placeholder="Your name"/></label></div></header><div className="nm-owner-grid"><aside className="nm-owner-list"><h2>Review owners</h2>{owners.map((x:any,i:number)=><button className={x.id===owner?.id?"active":""} onClick={()=>setActive(x.id)} key={x.id}><strong>{i+1}. {x.reviewerName||x.recordedNameRaw||"Source owner"}</strong><span>Page {x.pageStart}</span><small>{x.reviewState==="Reviewed"?"Reviewed":x.reviewState==="SourceUnclear"?"Source unclear":"Needs review"}</small></button>)}</aside><section className="nm-owner-form">{draft&&owner&&<><h1>Recorded person as per NM</h1><div className="nm-fields"><label>Name<input value={draft.name} onChange={e=>setDraft({...draft,name:e.target.value})}/><small>Extracted: {owner.recordedNameRaw||"—"}</small></label><label>Father / spouse<input value={draft.father} onChange={e=>setDraft({...draft,father:e.target.value})}/><small>Extracted: {owner.fatherOrSpouseRaw||"—"}</small></label><label>Residence<input value={draft.residence} onChange={e=>setDraft({...draft,residence:e.target.value})}/><small>Extracted: {owner.residenceRaw||"—"}</small></label><label>Share<input value={draft.share} onChange={e=>setDraft({...draft,share:e.target.value})}/><small>Extracted: {owner.shareRaw||"Missing"}</small></label></div><h2>Land</h2>{dittoAmbiguous&&<p className="nm-land-review">Land details need review</p>}{draft.parcels.map((p:any,i:number)=><div className="nm-owner-parcel" key={p.id}><label>Khasra<input value={p.khasra} onChange={e=>setDraft({...draft,parcels:draft.parcels.map((q:any,n:number)=>n===i?{...q,khasra:e.target.value}:q)})}/><small>Extracted: {p.source.rawKhasraText||"—"}</small></label><label>Area<input value={p.area} onChange={e=>setDraft({...draft,parcels:draft.parcels.map((q:any,n:number)=>n===i?{...q,area:e.target.value}:q)})}/><small>Extracted: {p.source.rawAreaText||"Missing"}</small></label><label>Land class<input value={p.landClass} onChange={e=>setDraft({...draft,parcels:draft.parcels.map((q:any,n:number)=>n===i?{...q,landClass:e.target.value}:q)})}/><small>{p.source.isInherited?"Same as previous source owner · Inherited":`Extracted: ${p.source.landClassRaw||"—"}`}</small></label></div>)}<section className="nm-owner-summary"><h2>Summary</h2><div><span>Kita</span><strong>{owner.summary?.parcelCountAsRecorded||"Needs review"}</strong></div><div><span>Total Area</span><strong>{owner.summary?.totalAreaAsRecorded||"Needs review"}</strong></div></section><section className="nm-owner-compensation"><h2>Compensation</h2><div className="nm-compensation-grid">{(owner.compensation||[]).map((component:any)=><article key={component.componentType} className={component.isResolved?"":"unresolved"}><span>{component.label}</span><strong>{component.isResolved?amount(component.amount):"Needs review"}</strong><small>{component.rawAmountText?`${component.isResolved?"Source":"Source text"}: ${component.rawAmountText}${component.sourcePage?` · Page ${component.sourcePage}`:""}`:"Not safely staged from source"}</small></article>)}</div>{owner.runningTotals?.length>0&&<details className="nm-running-total"><summary>Source Running Total</summary>{owner.runningTotals.map((component:any)=><p key={component.componentType}>{component.rawAmountText||"—"} · {component.sourcePage?`Page ${component.sourcePage}`:"Source page unavailable"}</p>)}</details>}</section><details><summary>Diagnostics</summary>{owner.exceptions.map((x:any)=><p key={x.id}>{x.reason}: {x.detail}</p>)}</details><footer><button className="quiet-button" onClick={()=>setActive(owners[Math.max(0,owners.findIndex((x:any)=>x.id===owner.id)-1)]?.id)}>Previous owner</button><button disabled={saving||!reviewer.trim()} onClick={()=>void save()}>Save owner & Next</button><button className="quiet-button" disabled={saving||!reviewer.trim()} onClick={()=>void save(true)}>Source unclear</button></footer></>}</section><aside className="nm-owner-source">{owner&&data.data?.nmDocumentId&&<DocumentPdfViewer key={`${owner.id}-${owner.pageStart}`} documentId={data.data.nmDocumentId} initialPage={owner.pageStart}/>}</aside></div></main>}
void NmSemanticReviewWorkspaceV2;
function NmLegacyReviewRedirect(){const {id="",nmId=""}=useParams();return <Navigate replace to={`/awards/${id}/nm/${nmId}/semantic-review`}/>}
