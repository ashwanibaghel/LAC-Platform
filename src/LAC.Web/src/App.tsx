...
function AwardDocumentsSection({awardId}:{awardId:string}) {
  const [tick,setTick]=useState(()=>Date.now());const [message,setMessage]=useState("");const [preview,setPreview]=useState<any>();
  const result=useApi<any[]>(`/awards/${awardId}/documents?r=${tick}`);
  useEffect(()=>{
    // Do not poll while a PDF is open: changing the document list can replace
    // the embedded viewer and reset the user's current page/zoom/selection.
    if(preview || !result.data?.some(doc=>doc.job && ["Queued","Extracting","Analyzing","BuildingCandidates"].includes(doc.job.status))) return;
    const timer=window.setInterval(()=>setTick(Date.now()),5000);
    return()=>window.clearInterval(timer);
  },[preview,result.data]);
  const analyze=async(doc:any)=>{try{setMessage("");const villageId=doc.villages?.length===1?doc.villages[0].villageId:undefined;await post(`/awards/${awardId}/documents/${doc.id}/analyze`,{villageId});setTick(Date.now());}catch(e){setMessage(e instanceof Error?e.message:"Could not start analysis.");}};
  const reanalyze=async(id:string)=>{try{setMessage("");await post(`/award-pdf-extractions/${id}/reanalyze`,{});setTick(Date.now());}catch(e){setMessage(e instanceof Error?e.message:"Could not re-analyze saved pages.");}};
  if(result.error)return <section className="section"><h2>Documents</h2><p role="alert">{result.error}</p></section>;
  if(!result.data?.length)return null;
  const formatEta=(seconds:number|null)=>seconds===null?"Estimating remaining time…":seconds<60?`About ${Math.max(1,Math.round(seconds))} sec left`:`About ${Math.ceil(seconds/60)} min left`;
  return <section className="award-documents"><div className="section-heading"><div><p className="section-eyebrow">Award record</p><h2>Documents</h2></div><span className="hint">Analysis runs in the background</span></div>{result.data.map(doc=>{const j=doc.job;const processing=j&&["Queued","Extracting","Analyzing","BuildingCandidates"].includes(j.status);const total=Number(j?.totalPages||0);const processed=Math.min(Number(j?.processedPages||0),total||Number.MAX_SAFE_INTEGER);const percent=processing&&total?Math.round(processed/total*100):0;const started=j?.startedAt?new Date(j.startedAt).getTime():0;const elapsed=started?Math.max(0,(Date.now()-started)/1000):0;const eta=processing&&processed>0&&total>processed?elapsed/processed*(total-processed):null;const state=!j?"Not analyzed":processing?`${j.currentStage||"Analyzing"} · ${processed}/${total||"…"} pages`:j.status==="Failed"?"Analysis needs attention":j.reviewed?"Reviewed":j.ingestionSessionId?`Review ready · ${j.attention} need attention`:"Stored";return <div className="award-document-row" key={doc.id}><div className="document-icon">PDF</div><div className="document-main"><strong>{doc.originalFileName}</strong><span>{state}</span>{processing&&<div className="document-progress" aria-label={`${percent}% complete`}><div className="document-progress-track"><span style={{width:total?`${Math.max(2,percent)}%`:"35%"}} /></div><div className="document-progress-meta"><span>{total?`${percent}% complete`:"Preparing…"}</span><span>{formatEta(eta)}</span></div></div>}</div><div className="document-actions"><button className="quiet-button" onClick={()=>setPreview(doc)}>View PDF</button>{!j&&<button onClick={()=>analyze(doc)}>Analyze data</button>}{j?.ingestionSessionId&&<Link className="primary-link" to={`/awards/${awardId}/ingestion/${j.ingestionSessionId}`}>Review data</Link>}{j&&!processing&&<button className="quiet-button" onClick={()=>reanalyze(j.id)}>Re-analyze</button>}</div></div>;})}{message&&<p className="form-message">{message}</p>}{preview&&<aside className="document-viewer-modal" role="dialog" aria-modal="true" aria-label="Award document"><header><strong>{preview.originalFileName}</strong><button className="quiet-button" onClick={()=>setPreview(undefined)}>Close</button></header><DocumentPdfViewer documentId={preview.id}/></aside>}</section>;
}
...
function FactVerificationDrawer({candidate,documentId,awardId,reviewer,reason,onClose,onSaved}:{candidate:any;documentId:string;awardId:string;reviewer:string;reason:string;onClose:()=>void;onSaved:()=>void}) {
  ...
  const source=page&&documentId?`${api}/documents/${documentId}/content#page=${page}&view=FitH&navpanes=0`:undefined;
  ...
  return <aside className="evidence-workbench" role="dialog" aria-modal="true" aria-label="Verify source fact"><form className="evidence-editor" onSubmit={e=>{e.preventDefault();if(supported&&!busy&&awardId&&officer.trim()&&!candidate.verifiedAt)void save("Confirm");}}>
    ...
    <div className="form-footer"><label className="review-officer">Verifying officer<input value={officer} onChange={e=>{setOfficer(e.target.value);sessionStorage.setItem("lac.reviewOfficer",e.target.value);}} placeholder="Your name"/></label>{!candidate.verifiedAt&&<>{supported&&<button type="submit" disabled={busy||!awardId||!officer.trim()}>{busy?"Saving…":"Confirm & next →"}</button>}{candidate.candidateType==="AwardKhasra"&&<button type="button" disabled={busy||!awardId||!officer.trim()} onClick={()=>save("LinkExisting")}>Link existing</button>}<button type="button" disabled={busy||!officer.trim()} onClick={()=>save("Skip")}>Skip</button></>}</div>
