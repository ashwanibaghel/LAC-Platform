...
function NmUploadCard({awardId}:{awardId:string}){const [file,setFile]=useState<File>();const [message,setMessage]=useState("");const [busy,setBusy]=useState(false);const nms=useApi<any[]>(`/awards/${awardId}/nm-documents`);const submit=async()=>{if(!file)return;try{setBusy(true);await uploadNmPdf(awardId,file);setMessage("NM uploaded and linked. Review is ready; no OCR or official facts were created.");}catch(e){setMessage(e instanceof Error?e.message:"NM upload failed.");}finally{setBusy(false);}};return <div className="award-document-row"><div className="document-icon">NM</div><div className="document-main"><strong>NM / recorded-person register</strong><span>Village and Award are fixed by this workspace. Upload does not start analysis.</span>{nms.data?.map(n=><small key={n.id}>{n.originalFileName} · {n.status} · {n.rows} source rows</small>)}</div><div className="document-actions"><input type="file" accept="application/pdf,.pdf" onChange={e=>setFile(e.target.files?.[0])}/><button disabled={!file||busy} onClick={submit}>{busy?"Uploading…":"Add NM"}</button></div>{message&&<p className="form-message">{message}</p>}</div>}
