...
function Village() {
  ...
  return (
    <>
      ...
      <div className="summary-strip village-summary-strip">
...
function Award() {
  ...
  return (
    <>
      ...
      <AwardMatters awardId={id} villages={a.villages} />
...
function AwardMatters({ awardId, villages }: { awardId: string; villages: any[] }) {
  const villageId = villages.length === 1 ? villages[0]?.id : undefined;
  const matters = useApi<any[]>(villageId ? `/villages/${villageId}/matters` : undefined);
  const linked = matters.data?.filter((item) => item.award?.awardId === awardId) || [];
  if (!villageId || (!matters.loading && !linked.length)) return null;
  return <section className="section award-matters">
    <div className="section-heading"><div><h2>Related Matters</h2><span>Case files linked to this Award.</span></div></div>
    {matters.loading ? <LoadingState label="Loading related matters…" /> : <div className="matter-row-list">{linked.map((matter) => <div className="matter-row" key={matter.id}><EntityLink to={`/matters/${matter.id}`}>{matter.title}</EntityLink><span>{matter.matterType}</span><StatusBadge>{matter.status}</StatusBadge></div>)}</div>}
  </section>;
}
