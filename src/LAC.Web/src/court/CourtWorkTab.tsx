...
export const CourtWorkTab: React.FC<CourtWorkTabProps> = ({ courtCase, onRefresh }) => {
  const canLinkMatter = courtCase.capabilities?.canLinkMatter ?? false;

  const [workData, setWorkData] = useState<CourtCaseLinkedWorkDto | null>(null);
  const [loading, setLoading] = useState(courtCase.mattersCount !== null);
  ...
  const fetchWork = useCallback(async () => {
    if (courtCase.mattersCount === null) {
      setLoading(false);
      return;
    }
    try {
  ...
  }, [courtCase.id, courtCase.mattersCount]);
  ...
  const handleUnlinkMatter = async (matterId: string) => {
    ...
    try {
      const url = courtCase.revision != null
        ? `/api/court-cases/${courtCase.id}/matters/${matterId}?expectedRevision=${courtCase.revision}`
        : `/api/court-cases/${courtCase.id}/matters/${matterId}`;
