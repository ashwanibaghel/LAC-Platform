...
public static class WorkItemEndpoints
{
    ...
    public static RouteGroupBuilder MapWorkItemEndpoints(this RouteGroupBuilder api)
    {
        ...
        group.MapGet("/branch-pulse", async (string? q, Guid? workstreamId, Guid? deskId, string? attention, string? priority, int? staleDays, int? page, int? pageSize, LacDbContext db, IWorkItemAuthorizationService auth, ICurrentUserContext currentUser, IOfficeClock officeClock, CancellationToken ct) =>
        {
            ...
            IQueryable<WorkItem> query = db.WorkItems.AsNoTracking().Include(w => w.Workstream).Include(w => w.Assignments).ThenInclude(a => a.OfficeDesk).Include(w => w.Assignments).ThenInclude(a => a.AssignedUser).Include(w => w.Contributors).Where(w => w.RecordStatus == RecordStatus.Active && w.Assignments.Any(a => a.IsActive && a.RecordStatus == RecordStatus.Active) && (scopes.Contains(ScopeMode.All) || (scopes.Contains(ScopeMode.Workstream) && wsIds.Contains(w.WorkstreamId)) || (scopes.Contains(ScopeMode.Assigned) && w.Assignments.Any(a => a.IsActive && a.RecordStatus == RecordStatus.Active && deskIds.Contains(a.OfficeDeskId)))));
            // Filter options describe the caller's authorized operational area, not just this page.
            var workstreamOptions = await query.Select(w => new { w.WorkstreamId, workstreamName = w.Workstream.Name })
                .Distinct().OrderBy(w => w.workstreamName).Take(100).ToListAsync(ct);
            ...
            return Results.Ok(new { summary = new { open = open.Count, overdue = open.Count(Overdue), dueToday = open.Count(DueToday), needsReview = open.Count(NeedsReview), waitingOnHelp = open.Count(Waiting), notStarted = open.Count(w => w.Status == WorkItemStatus.Assigned), noNamedHandler = open.Count(NoHandler), stale = open.Count(Stale) }, workstreams = workstreamOptions.Select(w => new { workstreamId = w.WorkstreamId, w.workstreamName }), deskWorkloads = workload, items, totalCount = total, page = number, pageSize = size, effectiveStaleDays = days });
        });
