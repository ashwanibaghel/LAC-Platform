...
api.MapPost("/nm-documents/{id:guid}/analyze-semantic", async(Guid id,int[]? pages,NmWorkflowService workflow,CancellationToken ct)=> { try { var session = await workflow.AnalyzeSemanticAsync(id,ct,pages); return Results.Ok(new { session.Id, session.Status, session.AutoStructuredCount, session.ExceptionCount }); } catch(NmWorkflowException ex) { return Results.Problem(ex.Message,statusCode:ex.StatusCode); } });
