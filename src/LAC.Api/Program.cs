...
api.MapGet("/villages/{id:guid}/overview", async (Guid id, LacDbContext db, ICourtAuthorizationService courtAuth, ICurrentUserContext currentUser, CancellationToken ct) =>
{
    var village = await db.Villages.AsNoTracking().Where(x => x.Id == id).Select(x => new VillageDetail(x.Id, x.Name,
        new SubDivisionReference(x.SubDivision.Id, x.SubDivision.Name, new DistrictReference(x.SubDivision.District.Id, x.SubDivision.District.Name)),
        x.Khasras.Count, x.Khasras.SelectMany(k => k.AwardLinks).Select(link => link.AwardId).Distinct().Count(),
    ...
    var canViewCourt = currentUser.UserId.HasValue && await courtAuth.CanViewCourtReferencesAsync(currentUser.UserId.Value, ct);
    var courtCount = canViewCourt ? await db.Set<CourtCaseAward>().CountAsync(link => awardIdList.Contains(link.AwardId), ct) : 0;
...
api.MapGet("/khasras/{id:guid}/history", async (Guid id, LacDbContext db, ICourtAuthorizationService courtAuth, ICurrentUserContext currentUser, CancellationToken ct) =>
{
    ...
    var canViewCourt = currentUser.UserId.HasValue && await courtAuth.CanViewCourtReferencesAsync(currentUser.UserId.Value, ct);
    var court = canViewCourt
        ? await db.Set<CourtCaseKhasra>().AsNoTracking().Where(link => link.KhasraId == id).OrderBy(link => link.CourtCase.CaseNumber)
...
api.MapGet("/awards/{id:guid}/workspace", async (Guid id, LacDbContext db, ICourtAuthorizationService courtAuth, ICurrentUserContext currentUser, CancellationToken ct) =>
{
    var item = await db.Awards.AsNoTracking().Where(x => x.Id == id).Select(x => new {
        x.Id, x.AwardNumber, x.AwardDate, x.AwardType, x.ParentAwardId, ParentAwardNumber = x.ParentAward == null ? null : x.ParentAward.AwardNumber, x.ParentAwardReference, x.Purpose, x.ActRegime, x.Status, x.Remarks,
        AcquisitionProject = x.AcquisitionProject == null ? null : new ProjectReference(x.AcquisitionProject.Id, x.AcquisitionProject.Name, x.AcquisitionProject.RequiringAgency, x.AcquisitionProject.ActRegime),
        Villages = x.VillageLinks.Select(v => new VillageReference(v.Village.Id, v.Village.Name, new SubDivisionReference(v.Village.SubDivision.Id, v.Village.SubDivision.Name, new DistrictReference(v.Village.SubDivision.District.Id, v.Village.SubDivision.District.Name)))).ToList(),
    ...
    var canViewCourt = currentUser.UserId.HasValue && await courtAuth.CanViewCourtReferencesAsync(currentUser.UserId.Value, ct);
    var courtCount = canViewCourt ? item.CourtCasesCount : 0;
    var courtStatus = canViewCourt ? (item.HasCourtCases ? "Available" : "Not Added") : "Not Added";

    var result = new AwardWorkspaceOverview(
        item.Id, item.AwardNumber, item.AwardDate, item.AwardType, item.ParentAwardId, item.ParentAwardNumber, item.ParentAwardReference, item.Purpose, item.ActRegime, item.Status, item.Remarks,
        item.AcquisitionProject,
        item.Villages,
        item.KhasraLinksCount, item.NotificationLinksCount, item.PossessionEventsCount, courtCount, item.ClaimsCount, item.AreaIssuesCount, item.DocumentsCount,
        item.HasKhasras ? "Available" : "Not Added",
        item.HasNotifications ? "Available" : "Not Added",
        item.HasPossession ? "Partial" : "Not Added",
        courtStatus,
        item.HasClaims ? "Available" : "Not Added"
    );
...
api.MapGet("/villages/{id:guid}/core-records", async (Guid id, LacDbContext db, CancellationToken ct) =>
{
    ...
    var jobs = await db.AwardDocumentExtractionJobs.AsNoTracking()
        .Where(j => j.TargetAwardId != null && awardIds.Contains(j.TargetAwardId.Value))
        .OrderByDescending(j => j.CreatedAt)
        .ToListAsync(ct);

    var sessions = await db.AwardIngestionSessions.AsNoTracking()
        .Include(s => s.Candidates)
        .Where(s => s.TargetAwardId != null && awardIds.Contains(s.TargetAwardId.Value))
        .OrderByDescending(s => s.CreatedAt)
        .ToListAsync(ct);

    static bool IsUnresolved(AwardIngestionCandidate c) =>
        c.VerifiedAt == null &&
        c.Status != AwardIngestionCandidateStatus.Committed &&
        c.Status != AwardIngestionCandidateStatus.Skipped &&
        c.Status != AwardIngestionCandidateStatus.Rejected;
    ...
    return Results.Ok(awards.Select(a =>
    {
        ...
        return new
        {
            ...
            extractionStatus = latestJob?.Status.ToString(),
            ...
            {
                role,
                count = a.documents.Count(d => d.CoreDocumentRole == role),
                available = a.documents.Any(d => d.CoreDocumentRole == role)
            }),
            documents = a.documents.Select(d =>
            {
                ...
                return new
                {
                    ...
                    d.OriginalFileName,
                    d.UploadedAt,
                    extractionJobId = docJob?.Id,
                    extractionStatus = docJob?.Status.ToString(),
                    totalPages = docJob?.TotalPages,
                    processedPages = docJob?.ProcessedPages ?? 0,
                    currentStage = docJob?.CurrentStage,
...
api.MapPut("/awards/{id:guid}", async (Guid id, UpdateAwardRequest request, LacDbContext db, ICurrentUserContext currentUser, CancellationToken ct) =>
{
    if (string.IsNullOrWhiteSpace(request.AwardNumber)) return Validation("awardNumber", "Award number is required.");
    var award = await db.Awards.FirstOrDefaultAsync(a => a.Id == id, ct);
    if (award is null) return NotFound("Award", id);

    var oldValues = JsonSerializer.Serialize(new { award.AwardNumber, AwardDate = award.AwardDate?.ToString("yyyy-MM-dd"), award.AwardType });
    ...
    award.UpdatedBy = actor;

    var newValues = JsonSerializer.Serialize(new { award.AwardNumber, AwardDate = award.AwardDate?.ToString("yyyy-MM-dd"), award.AwardType });
...
api.MapPost("/awards/{id:guid}/core-documents", async (Guid id, string role, IFormFile file, LacDbContext db, IDocumentStorage storage, ICurrentUserContext currentUser, CancellationToken ct) =>
{
    ...
    var actor = currentUser.DisplayName ?? currentUser.Username ?? currentUser.UserId?.ToString() ?? "System";
    var document = new Document { DocumentType = role, OriginalFileName = file.FileName, StoragePath = stored.StoragePath, Sha256Hash = stored.Sha256Hash, FileSize = stored.FileSize, MimeType = file.ContentType, UploadedAt = DateTimeOffset.UtcNow, UploadedBy = actor };
...
api.MapPut("/awards/{id:guid}/core-documents/{documentId:guid}", async (
    Guid id,
    Guid documentId,
    string? reason,
    IFormFile file,
    ...
    CancellationToken ct) =>
{
    ...
    var link = await db.DocumentAwards.FirstOrDefaultAsync(x => x.AwardId == id && x.DocumentId == documentId && x.CoreDocumentRole != null, ct);
    if (link is null) return Validation("documentId", "Target core document relationship not found for this Award.");

    var role = link.CoreDocumentRole!;
    var villageIds = await db.AwardVillages.Where(x => x.AwardId == id).Select(x => x.VillageId).Distinct().ToListAsync(ct);
    if (villageIds.Count == 0) return NotFound("Award", id);
    ...
    CancellationToken ct) =>
{
    ...
    db.Add(new AuditLog
    {
        ...
        ChangedBy = actor,
        ChangedAt = DateTimeOffset.UtcNow,
        OldValues = $"Role '{role}', UnlinkedDocId={documentId}",
        NewValues = $"Role '{role}' removed from Award #{award.AwardNumber}. UnlinkedDocId={documentId}. Reason={reason.Trim()}"
    });
...
api.MapGet("/search", async (string? q, LacDbContext db, ICourtAuthorizationService courtAuth, ICurrentUserContext currentUser, CancellationToken ct) =>
{
    ...
    var canViewCourt = currentUser.UserId.HasValue && await courtAuth.CanViewCourtReferencesAsync(currentUser.UserId.Value, ct);
    var courtCases = new List<SearchResultItem>();
    if (canViewCourt && currentUser.UserId.HasValue)
    {
        var courtQuery = db.CourtCases.AsNoTracking().Where(x => x.RecordStatus == RecordStatus.Active);
        courtQuery = await courtAuth.AuthorizeListQueryAsync(courtQuery, currentUser.UserId.Value, ct);
        courtCases = await courtQuery
            .Where(x => x.CaseNumber.ToUpper().Contains(term) || (x.CaseTitle != null && x.CaseTitle.ToUpper().Contains(term)) || x.CourtName.ToUpper().Contains(term))
            .Take(8)
            .Select(x => new SearchResultItem("Court Case", x.Id, x.CaseNumber, x.CourtName + (x.CaseTitle != null ? " · " + x.CaseTitle : ""), $"/court-cases/{x.Id}"))
            .ToListAsync(ct);
    }

    return Results.Ok(villages.Concat(khasras).Concat(awards).Concat(parties).Concat(courtCases));
}).RequirePermission(PermissionCodes.VillageView);
...
static string? GetAuthenticatedActor(ICurrentUserContext currentUser) =>
    currentUser.IsAuthenticated
        ? currentUser.DisplayName ?? currentUser.Username ?? currentUser.UserId?.ToString()
        : null;

api.MapPost("/award-ingestion-sessions/{id:guid}/context", async (Guid id,ReviewContextRequest request,AwardIngestionService ingestion,ICurrentUserContext currentUser,CancellationToken ct) => {try{var actor=GetAuthenticatedActor(currentUser);if(string.IsNullOrWhiteSpace(actor))return Results.Unauthorized();request=request with {VerifiedBy=actor};await ingestion.SetReviewContextAsync(id,request,ct);return Results.NoContent();}catch(AwardIngestionException ex){return IngestionProblem(ex);}}).RequirePermission(PermissionCodes.AwardEdit, WorkstreamCodes.Award);
api.MapPost("/award-ingestion-sessions/{id:guid}/confirm-exact", async (Guid id,ConfirmExactRequest request,AwardIngestionService ingestion,ICurrentUserContext currentUser,CancellationToken ct) => {try{var actor=GetAuthenticatedActor(currentUser);if(string.IsNullOrWhiteSpace(actor))return Results.Unauthorized();request=request with {VerifiedBy=actor};return Results.Ok(new {confirmed=await ingestion.ConfirmExactAsync(id,request,ct)});}catch(AwardIngestionException ex){return IngestionProblem(ex);}}).RequirePermission(PermissionCodes.AwardEdit, WorkstreamCodes.Award);
api.MapPost("/award-ingestion-sessions/{id:guid}/commit-verified", async (Guid id,CommitVerifiedRequest request,AwardIngestionService ingestion,ICurrentUserContext currentUser,CancellationToken ct) => {try{var actor=GetAuthenticatedActor(currentUser);if(string.IsNullOrWhiteSpace(actor))return Results.Unauthorized();request=request with {VerifiedBy=actor};return Results.Ok(await ingestion.CommitVerifiedAsync(id,request,ct));}catch(AwardIngestionException ex){return IngestionProblem(ex);}}).RequirePermission(PermissionCodes.AwardEdit, WorkstreamCodes.Award);
api.MapPost("/award-ingestion-candidates/{id:guid}/verify", async (Guid id,VerifyExtractedFactRequest request,AwardIngestionService ingestion,ICurrentUserContext currentUser,CancellationToken ct) => {try{var actor=GetAuthenticatedActor(currentUser);if(string.IsNullOrWhiteSpace(actor))return Results.Unauthorized();request=request with {VerifiedBy=actor};await ingestion.VerifyFactAsync(id,request,ct);return Results.NoContent();}catch(AwardIngestionException ex){return IngestionProblem(ex);}}).RequirePermission(PermissionCodes.AwardEdit, WorkstreamCodes.Award);
api.MapPost("/award-ingestion-candidates/{id:guid}/verify-award-khasra-field", async (Guid id, VerifyAwardKhasraFieldRequest request, AwardIngestionService ingestion, ICurrentUserContext currentUser, CancellationToken ct) => { try { var actor=GetAuthenticatedActor(currentUser);if(string.IsNullOrWhiteSpace(actor))return Results.Unauthorized();request=request with {VerifiedBy=actor};await ingestion.VerifyAwardKhasraFieldAsync(id, request, ct); return Results.NoContent(); } catch (AwardIngestionException ex) { return IngestionProblem(ex); } }).RequirePermission(PermissionCodes.AwardEdit, WorkstreamCodes.Award);
...
api.MapPost("/award-ingestion-candidates/{id:guid}/resolve", async (Guid id, ResolveAwardIngestionCandidateRequest request, AwardIngestionService ingestion, ICurrentUserContext currentUser, CancellationToken ct) => { try { var actor=GetAuthenticatedActor(currentUser);if(string.IsNullOrWhiteSpace(actor))return Results.Unauthorized(); await ingestion.ResolveAsync(id, request.Action, actor, ct); return Results.NoContent(); } catch (AwardIngestionException ex) { return IngestionProblem(ex); } }).RequirePermission(PermissionCodes.AwardEdit, WorkstreamCodes.Award);
api.MapPost("/award-ingestion-sessions/{id:guid}/commit", async (Guid id, CommitAwardIngestionSessionRequest request, AwardIngestionService ingestion, ICurrentUserContext currentUser, CancellationToken ct) => { try { var actor=GetAuthenticatedActor(currentUser);if(string.IsNullOrWhiteSpace(actor))return Results.Unauthorized(); return Results.Ok(await ingestion.CommitAsync(id, request.CandidateIds, actor, ct)); } catch (AwardIngestionException ex) { return IngestionProblem(ex); } }).RequirePermission(PermissionCodes.AwardEdit, WorkstreamCodes.Award);
...
api.MapPost("/awards/{id:guid}/court-cases", async (Guid id, CreateAwardCourtCaseRequest request, LacDbContext db, ICourtWorkflowService courtWorkflow, ICourtAuthorizationService courtAuth, ICurrentUserContext currentUser, CancellationToken ct) =>
{
    ...
    try
    {
        ...
        var item = await courtWorkflow.CreateCourtCaseAsync(cmd, currentUser.UserId.Value, ct);
        return Results.Created($"/api/court-cases/{item}", new IdResponse(item));
    }
