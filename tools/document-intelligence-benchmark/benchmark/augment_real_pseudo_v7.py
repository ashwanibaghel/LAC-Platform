...
def main():
 ...
 for item in json.loads(a.labels.read_text()):
  for n in range(a.variants):
   im=Image.open(item['sourceCrop']).convert('L'); im=im.resize((max(8,int(im.width*r.uniform(.9,1.1))),max(8,int(im.height*r.uniform(.9,1.1)))),Image.Resampling.LANCZOS); im=ImageEnhance.Contrast(im).enhance(r.uniform(.8,1.25)); im=ImageEnhance.Brightness(im).enhance(r.uniform(.85,1.15));
   if n%2: im=im.filter(ImageFilter.GaussianBlur(.35))
   p=a.output/f'{Path(item["sourceCrop"]).stem}-v{n}.png'; im.save(p); out.append({'image':str(p),'text':item['text'],'role':item['role'],'sourcePage':item['sourcePage'],'sourceRow':item['sourceRow']})
