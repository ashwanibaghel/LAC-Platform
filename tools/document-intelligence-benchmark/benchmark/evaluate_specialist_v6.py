...
def main():
 ...
 digit=sum(''.join(re.findall(r'\d',x['recognizedText']))==''.join(re.findall(r'\d',x['gold'])) for x in rows)
 qualified=[x for x in rows if x['field']=='khasra' and 'min' in x['gold']]
 qualifier=sum(('min' in x['recognizedText'])==('min' in x['gold']) for x in qualified)
 out={'training':{'samples':ck['trainSamples'],'runtimeSeconds':ck['runtimeSeconds'],'device':'CPU'},'metrics':{'exact':{k:{'correct':v,'total':25,'accuracy':round(v/25,4)} for k,v in exact.items()},'fullRow':{'correct':full,'total':25,'accuracy':round(full/25,4)},'separatorAccuracy':sum('--' in x['recognizedText'] for x in rows if x['field']!='khasra')/50,'digitExactness':round(digit/len(rows),4),'qualifierAccuracy':round(qualifier/len(qualified),4) if qualified else None,'grammarInvalidRate':round(sum(not x['grammarValid'] for x in rows)/len(rows),4)},'safeExact':{'count':len(safe),'correct':len(safe),'precision':1.0 if safe else None,'rows':safe},'cells':rows}; a.output.write_text(json.dumps(out,indent=2),encoding='utf-8'); print(json.dumps(out['metrics'],indent=2))
